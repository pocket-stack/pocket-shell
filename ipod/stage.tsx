...
// ipod/stage.tsx — the live miniature of the focused monitor:
...
import type { CompanionStore, TileSlot } from "./store.ts";
...
function Tile(p: { store: CompanionStore; slot: TileSlot }) {
  const s = p.slot;
 ...
 */
export function LaunchBar(p: { store: CompanionStore }) {
  return (
    <View
...
export function Stage(p: { store: CompanionStore }) {
  const empty = () => {
...
export function TilePopup(p: { store: CompanionStore }) {
  return (
...
function stageTarget(store: CompanionStore, x: number, y: number): Target {
...
export function stageHandlers(store: CompanionStore): GestureHandlers {
...
export function popupHandlers(store: CompanionStore): GestureHandlers {
  let outside = false;
  return {
    onDown: (c) => {
      const popup = store.popup();
