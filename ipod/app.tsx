...
// ipod/app.tsx — Pocket Shell: an Omarchy companion for the
...
import { connectSvc, createCompanionStore, type Svc } from "./store.ts";
...
function Toast(p: { store: ReturnType<typeof createCompanionStore> }) {
  return (
...
export default function PocketRemote() {
  // The headless sim (test/ipod-sim.test.ts) has no svc channel;
  ...
  const store = createCompanionStore((globalThis as { __pocketShellIpodSvc?: Svc }).__pocketShellIpodSvc ?? connectSvc());
  (globalThis as { __pocketShellIpod?: unknown }).__pocketShellIpod = store;
