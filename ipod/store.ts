...
// ipod/store.ts — the remote's live state: the link to the
...
export type CompanionStore = ReturnType<typeof createCompanionStore>;

// ---------------------------------------------------------------------------
// store
// ---------------------------------------------------------------------------

export function createCompanionStore(svc: Svc | null = connectSvc()) {
  // ---- link -----------------------------------------------------------------
  ...
  onFrame(() => {
    ...
    if (svc) {
      ...
      if (up && !opened) {
        opened = true;
        svc.send({ t: "hello", proto: SHELL_PROTO, device: deviceName() });
      } else if (!up && opened) {
        opened = false;
        setLink("search");
        setState(null);
