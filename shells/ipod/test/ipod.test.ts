...
// Pocket Shell on the iPod touch: the wire constants pinned to spec.ts, framing, protocol
...
import { ACTIONS, actionById, actionsOf, LAUNCHERS } from "../src/actions.ts";
import { GUTTER, ICON_BOX, RADIUS, ROW_H, rowMetrics, SPACE } from "../src/design.ts";
import { appName, windowLabels } from "../src/names.ts";
import { GLYPH } from "../src/glyphs.ts";
import * as wire from "../src/host/wire.ts";
import {
  buildState,
  cleanTitle,
  luaWindow,
  ...
  type HyprMonitor,
  type HyprWorkspace,
} from "../src/host/hypr.ts";
import { chordKey, parseBindings } from "../src/host/keymap.ts";
import { childrenOf, normalizeMenu, parentOf, parseMenuJsonc, stripJsonc } from "../src/host/menu-source.ts";
import {
...
import { MENU, MENU_OMARCHY_VERSION } from "../src/menu.ts";
import { MENU_DOT_EMOJI, MENU_ROOT, menuChildren, menuItem, menuParent, menuTitle, menuVisible } from "../src/menu-tree.ts";
import { clipTitle, parseLines, SHELL_APP, SHELL_PROTO, TITLE_MAX, WINDOWS_MAX, type HostState } from "../src/protocol.ts";
import { isThemeColors, themeTitle, TOKYO_NIGHT } from "../src/theme.ts";

describe("pocket-shell wire", () => {
  ...
  test("frames round-trip through the parser, hello parses incrementally", () => {
    ...
    const hello = new Uint8Array(7 + SHELL_APP.length);
    new DataView(hello.buffer).setUint32(0, WIRE_MAGIC, true);
    hello[4] = WIRE_VERSION;
    hello[6] = SHELL_APP.length;
    hello.set(new TextEncoder().encode(SHELL_APP), 7);
    expect(wire.parseHello(hello.subarray(0, 10))).toBeNull();
    expect(wire.parseHello(hello)).toEqual({ app: SHELL_APP, consumed: 7 + SHELL_APP.length });
  ...
  test("beacon layout: magic, version, port, app, name", () => {
    const beacon = wire.encodeBeacon(SHELL_APP, "x1nano", 8622);
    ...
    expect(beacon[8]).toBe(SHELL_APP.length);
    expect(new TextDecoder().decode(beacon.subarray(9, 9 + SHELL_APP.length))).toBe(SHELL_APP);
    expect(beacon[9 + SHELL_APP.length]).toBe(6);
  });
});

describe("pocket-shell protocol", () => {
...
describe("pocket-shell hypr mirror", () => {
...
describe("pocket-shell omarchy readers", () => {
...
describe("pocket-shell menu source", () => {
  const jsonc = `{
  // Root Menu
...
describe("pocket-shell baked menu", () => {
...
describe("pocket-shell design system", () => {
...
describe("pocket-shell wtype chords", () => {
