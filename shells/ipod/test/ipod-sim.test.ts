...
// Boots the built iPod bundle in the headless sim and drives it the
// way the daemon would: hello, theme, a Hyprland snapshot. No svc channel
// exists in the sim, so the app publishes its store as globalThis.__pocketShellIpod
...
import type { HostState } from "../src/protocol.ts";
import { CLICK_KEY, DPAD_ARMS, keyboardKeys, MENU_KEY, TRACKPAD } from "../src/keyboard-layout.ts";
import {
  BALL_HOME,
  CC_BUTTON,
  launchCellRect,
...
import type { CompanionStore } from "../src/store.ts";
...
async function connected(): Promise<{ world: Awaited<ReturnType<typeof bootWorld>>; store: CompanionStore; sent: unknown[] }> {
  const sent: unknown[] = [];
  const svc = { open: () => true, poll: () => [], send: (line: unknown) => sent.push(line) };
  const world = await bootWorld("pocketshell-ipod", HZ, { __pocketShellIpodSvc: svc }, undefined, { width: 480, height: 320 });
...
describe("pocket-shell in the sim", () => {
  test("connects, mirrors a snapshot into tiles, and switches workspace from the strip", async () => {
    ...
    expect(texts(world.getTree()).join(" ")).toContain("Pocket Shell");
