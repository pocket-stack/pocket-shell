...
Rendered in the headless sim by `bun run omarchy shots media`
...
| ![the click button held for a drag-select](media/deck-drag.png) | ![a d-pad arm held](media/deck-dpad.png) |
...
the machine's `omarchy-menu.jsonc` (`bun run omarchy menu x1nano`)
...
as Lua calls of one shape, and `src/host/keymap.ts` turns them into a chord map:
...
`src/host/pointer/pocket-pointer.c` is a client of that protocol and nothing
...
Lines are JSON (`src/protocol.ts`, `SHELL_PROTO` 2). Host → device: `hello`,
...
```
bun run omarchy deploy-host x1nano   # copy the daemon, build the pointer helper, install + start the user unit
bun run omarchy logs x1nano          # journal tail
bun run omarchy menu x1nano          # regenerate menu.ts after an Omarchy update
bun run omarchy shots out/           # render every screen in the headless sim
```
...
`ipodtouch4.json` carries the device-side identity (bundle, executable,
...
- `bun run omarchy client 127.0.0.1:8623 --for 4` — a scripted
