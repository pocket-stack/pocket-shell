...
//   bun run omarchy menu <ssh host>          regenerate shells/ipod/src/menu.ts from the machine's own menu
...
import { normalizeMenu, parseMenuJsonc, type MenuEntry } from "../src/host/menu-source.ts";
import { SHELL_APP, SHELL_PROTO, type ClientLine, type HostLine, parseLines } from "../src/protocol.ts";
import {
  encodeBeacon,
  encodeCtrl,
  encodeFrame,
...
const APP_DIR = join(REPOSITORY, "shells/ipod/src");
 ...
 * src/menu.ts: every row's id, parent, kind, icon, label and
...
// src/menu.ts — GENERATED by \`bun run omarchy menu\`
...
type Store = import("../src/store.ts").CompanionStore;
...
async function shots(outDir: string): Promise<void> {
  ...
  const { STAGE, TAB_W, CC_BUTTON, MODE, MODE_HALF_W, SHEET_LIST, sheetRowRect, BALL_HOME } = await import("../src/layout.ts");
  const { keyboardKeys } = await import("../src/keyboard-layout.ts");
  ...
  const { CLICK_KEY, DPAD_ARMS, TRACKPAD: PAD } = await import("../src/keyboard-layout.ts");
...
interface PanelParts {
  layout: typeof import("../src/layout.ts");
  keys: typeof import("../src/keyboard-layout.ts");
}
...
async function films(outDir: string, only?: string): Promise<void> {
  ...
  const parts: PanelParts = {
    layout: await import("../src/layout.ts"),
    keys: await import("../src/keyboard-layout.ts"),
  };
